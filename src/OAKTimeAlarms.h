...
#ifndef OAKTimeAlarms_h
#define OAKTimeAlarms_h
...
#else
#define dtNBR_ALARMS 25  // assume non-AVR has more memory
#endif
#endif
...
class OAKTimeAlarmsClass
{
private:
  OAKAlarmClass Alarm[dtNBR_ALARMS];
  void serviceAlarms();
  uint8_t isServicing;
  uint8_t servicedAlarmId; // the alarm currently being serviced
  AlarmID_t create(time_t value, voidFuncPtr onTickHandler, void *instance, uint8_t isOneShot, dtAlarmPeriod_t alarmType);
...
public:
  ...
  AlarmID_t triggerOnce(time_t value, voidFuncPtr onTickHandler, void *instance) {
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, true, dtExplicitAlarm);
  }

  // trigger once at given time of day
  AlarmID_t alarmOnce(time_t value, voidFuncPtr onTickHandler, void *instance) {
    if (value <= 0 || value > SECS_PER_DAY) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, true, dtDailyAlarm);
  }
  AlarmID_t alarmOnce(const int H, const int M, const int S, voidFuncPtr onTickHandler, void *instance) {
    return alarmOnce(AlarmHMS(H,M,S), onTickHandler, instance);
  }

  // trigger once on a given day and time
  AlarmID_t alarmOnce(const timeDayOfWeek_t DOW, const int H, const int M, const int S, voidFuncPtr onTickHandler, void *instance) {
    time_t value = (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S);
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, true, dtWeeklyAlarm);
  }

  // trigger daily at given time of day
  AlarmID_t alarmRepeat(time_t value, voidFuncPtr onTickHandler, void *instance) {
    if (value > SECS_PER_DAY) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, false, dtDailyAlarm);
  }
  AlarmID_t alarmRepeat(const int H, const int M, const int S, voidFuncPtr onTickHandler, void *instance) {
    return alarmRepeat(AlarmHMS(H,M,S), onTickHandler, instance);
  }

  // trigger weekly at a specific day and time
  AlarmID_t alarmRepeat(const timeDayOfWeek_t DOW, const int H, const int M, const int S, voidFuncPtr onTickHandler, void *instance) {
    time_t value = (DOW-1) * SECS_PER_DAY + AlarmHMS(H,M,S);
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, false, dtWeeklyAlarm);
  }

  // trigger once after the given number of seconds
  AlarmID_t timerOnce(time_t value, voidFuncPtr onTickHandler, void *instance) {
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, true, dtTimer);
  }
  AlarmID_t timerOnce(const int H, const int M, const int S, voidFuncPtr onTickHandler, void *instance) {
    return timerOnce(AlarmHMS(H,M,S), onTickHandler, instance);
  }

  // trigger at a regular interval
  AlarmID_t timerRepeat(time_t value, voidFuncPtr onTickHandler, void *instance) {
    if (value <= 0) return dtINVALID_ALARM_ID;
    return create(value, onTickHandler, instance, false, dtTimer);
  }
  AlarmID_t timerRepeat(const int H,  const int M,  const int S, voidFuncPtr onTickHandler, void *instance) {
    return timerRepeat(AlarmHMS(H,M,S), onTickHandler, instance);
  }
...
extern OAKTimeAlarmsClass Alarm;  // make an instance for the user
